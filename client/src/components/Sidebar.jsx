        ...
        <div className={`bg-[#8185b2]/10 h-full p-5  overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
            <div className="pb-5">
                <div className="flex justify-between items-center">
                    <img src={assets.logo} alt="logo" className='max-w-45' />
                ...
                <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
                    ...
                    <input onChange={(e)=>setInput(e.target.value)} value={input} type="text"  className=' text-sm bg-transparent border-none outline-none text-white placeholder-[#c8c8c8] flex-1' placeholder='Search User....' />
                </div>
            </div>
            <div className='flex flex-col'>
                {filteredUsers.map((user, index) => (
                    <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev=>({...prev,[user._id]:0})) }} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'} `}>
                        <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-10 aspect-square rounded-full' />
